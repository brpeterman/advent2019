module Advent
    class Wires
        def intersection(wire1, wire2)
            all_intersections(wire1, wire2).reduce({x: 0, y: 0}) do |closest, intersection|
                if (intersection[:x].abs + intersection[:y].abs) < (closest[:x].abs + closest[:y].abs) || closest == {x: 0, y: 0}
        ...
        def all_intersections(wire1, wire2)
            wire1_points = all_points(wire1)
            wire2_points = all_points(wire2)
            (wire1_points.keys & wire2_points.keys) - [{x: 0, y: 0}]
        end

        def all_points(wire_def)
            position = {x: 0, y: 0}
            wire_def.split(',').reduce({}) do |all_points, dist_and_dir|
                direction = dist_and_dir[0]
                amount = dist_and_dir[1..dist_and_dir.length-1].to_i
                case direction
                when 'R'
                    (position[:x]..position[:x]+amount).each do |x_pos|
                        all_points[{x: x_pos, y: position[:y]}] = true
                    end
                    position = {x: position[:x] + amount, y: position[:y]}
                when 'L'
                    (position[:x]-amount..position[:x]).each do |x_pos|
                        all_points[{x: x_pos, y: position[:y]}] = true
                    end
                    position = {x: position[:x] - amount, y: position[:y]}
                when 'U'
                    (position[:y]..position[:y]+amount).each do |y_pos|
                        all_points[{x: position[:x], y: y_pos}] = true
                    end
                    position = {x: position[:x] , y: position[:y] + amount}
                when 'D'
                    (position[:y]-amount..position[:y]).each do |y_pos|
                        all_points[{x: position[:x], y: y_pos}] = true
                    end
                    position = {x: position[:x] , y: position[:y] - amount}
                end
        ...
        def steps_to_intersect(wire1, wire2)
            intersections = all_intersections(wire1, wire2)
            # binding.pry
            intersections.reduce(-1) do |lowest, point|
                steps = steps_to_point(wire1, point) + steps_to_point(wire2, point)
                if (steps < lowest) || (lowest == -1)
                    lowest = steps
                end
                lowest
            end
        ...
        def steps_to_point(wire_def, destination)
            position = {x: 0, y: 0}
            steps = 0
            wire_def.split(',').each do |dist_and_dir|
                direction = dist_and_dir[0]
                amount = dist_and_dir[1..dist_and_dir.length-1].to_i
                case direction
                when 'R'
                    if(position[:y] == destination[:y]) && (position[:x] < destination[:x]) && (position[:x] + amount >= destination[:x])
                        steps += (destination[:x] - position[:x]).abs
                        break
                    else
                        steps += amount
                        position = {x: position[:x] + amount, y: position[:y]}
                ...
                when 'L'
                    if(position[:y] == destination[:y]) && (position[:x] > destination[:x]) && (position[:x] - amount <= destination[:x])
                        steps += (position[:x] - destination[:x]).abs
                        break
                    else
                        steps += amount
                        position = {x: position[:x] - amount, y: position[:y]}
                ...
                when 'U'
                    if(position[:x] == destination[:x]) && (position[:y] < destination[:y]) && (position[:y] + amount >= destination[:y])
                        steps += (destination[:y] - position[:y]).abs
                        break
                    else
                        steps += amount
                        position = {x: position[:x], y: position[:y] + amount}
                ...
                when 'D'
                    if(position[:x] == destination[:x]) && (position[:y] > destination[:y]) && (position[:y] - amount <= destination[:y])
                        steps += (position[:y] - destination[:y]).abs
                        break
                    else
                        steps += amount
                        position = {x: position[:x], y: position[:y] - amount}
